...
if($row=mysqli_fetch_assoc($d)){
	...
	echo "<script>window.location='bookllers_confbuydetails.php?bookid=".$bid."'</script>";
 }
 else{
  	echo "<script>alert('Wrong Verification'),window.location='bookllers.php';</script>";
  }
}
else{
	echo "<script>alert('Invalid Input. Please try again.'),window.location='bookllers.php';</script>";
}
?>