...
if(preg_match("/^[6-9]{1}[0-9]{9}+$/",$cntct) || preg_match("/^[6-9]{1}[0-9]{9}+$/",$whtsap))
{
   ...
   if($i>0)
   {
         ...
         echo "<script>alert('Registered successfully. Have a nice journey. Your Serial No.:$uid');window.location='bookllers.php';</script>";
   }
 }
 else{
   ...
   $correct="<h3 style='color:Red; '>Warning: You have Entered Wrong Number!!</h3>";
      header("location:bookllers_sellerregister.php?warning=".$correct);
 }
   ...
   $cor="<h3 style='color:Red; '>Warning: You have Entered Wrong Captcha!!</h3>";
      header("location:bookllers_sellerregister.php?warn=".$cor);
